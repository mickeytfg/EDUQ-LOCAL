...
for ($x = 0; $x < count($rowarray); $x++) {
	...
	foreach ( $rowarray[$x] as $review ) 
	{
		...
		//star number --------------------------------------
		$starfile_burl_logo_array = $templateclass->wprevpro_get_star_logo_burl($review,$imgs_url,$currentform[0],"t3");
		...
		if(	$currentform[0]->display_masonry=="yes"){
			$tempmasonrydiv ='wprs_masonry_item wprevpro_t3_DIV_1'.$tempwidget.' ';
		}		
