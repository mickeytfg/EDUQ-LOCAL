...
<div class="rt-vc-event rt-vc-event-grid">
	<?php if ( $events ): ?>
		...
		$total = sizeof( $events );
		$wp_query->max_num_pages = ceil( $total/ $grid_item_number );
		...
		<div class="row auto-clear">
			<?php foreach ( $events as $event ): ?>
				...
				if ( $event['start_date'] != $event['end_date'] ) {
					$event_time = "{$event['start_time']} - {$event['end_time']} ({$event['end_date']})";
				}
				else {
			...
			<?php endforeach;?>
			<div class="col-sm-12 col-xs-12 mt30"><?php RDTheme_Helper::pagination($count);?></div>
		</div>
	<?php else: ?>
