 ...
 * @Bean
 */
class EMSeCommerceCw_ExternalCheckoutService extends Customweb_Payment_ExternalCheckout_AbstractCheckoutService {

	public function loadContext($contextId, $cache = true){
		return EMSeCommerceCw_Entity_ExternalCheckoutContext::getContextById($contextId, $cache);
	}
	...
	protected function updateShippingMethodOnContext(Customweb_Payment_ExternalCheckout_IContext $context, Customweb_Core_Http_IRequest $request){
		if (!($context instanceof EMSeCommerceCw_Entity_ExternalCheckoutContext)) {
			throw new Customweb_Core_Exception_CastException('EMSeCommerceCw_Entity_ExternalCheckoutContext');
		}
		...
		if (isset($parameters['shipping_method']) && is_array($parameters['shipping_method'])) {
			...
			foreach ($packages as $i => $package) {
				if (!isset($package['rates'][$parameters['shipping_method'][$i]])) {
					throw new Exception(__("Not a valid shipping method", "woocommerce_emsecommercecw"));
				}
			...
			throw new Exception(__("Please select a shipping method.", "woocommerce_emsecommercecw"));
		}
	...
	private function getShippingMethodNameFromContext(EMSeCommerceCw_Entity_ExternalCheckoutContext $context){
		...
		elseif (count($selectedMethods) == 1) {
			...
			if (isset($available[$methodKey])) {
				...
				if(empty($title)){
					$title =  __("Shipping", "woocommerce_emsecommercecw");
				}
				return $title;
			}
			...
			if (isset($available['advanced_shipping'])) {
				if (is_numeric($methodKey) && get_post_type($methodKey) == 'was' && get_post_status($methodKey) == 'publish') {
					...
					$name = __("Shipping", "woocommerce_emsecommercecw");
			...
			//Woocommerce TableRate Shipping compatibility
			if(isset($available['betrs_shipping'])){
				if(substr( $methodKey, 0, strlen("betrs_shipping") ) === "betrs_shipping"){
					$shippingMethod = $available['betrs_shipping'];
					$title = $shippingMethod->get_title();
					if(empty($title)){
						$title =  __("Shipping", "woocommerce_emsecommercecw");
					}
	...
	protected function refreshContext(Customweb_Payment_ExternalCheckout_AbstractContext $context){
		...
		if ($context->getPaymentMethod() != null) {
			WC()->session->set('chosen_payment_method', 'EMSeCommerceCw_' . $context->getPaymentMethod()->class_name);
		}
	...
	protected function createTransactionContextFromContext(Customweb_Payment_ExternalCheckout_IContext $context){
		...
		$orderContext = new EMSeCommerceCw_OrderContext($order, new EMSeCommerceCw_PaymentMethodWrapper($paymentMethod));
	...
	public function authenticate(Customweb_Payment_ExternalCheckout_IContext $context, $emailAddress, $successUrl){
		...
		$externalCheckout = EMSeCommerceCw_ConfigurationAdapter::getExternalCheckoutAccountCreation();
	...
	public function renderShippingMethodSelectionPane(Customweb_Payment_ExternalCheckout_IContext $context, $errorMessages){
		...
		if (count($packages) == 1) {
			...
			EMSeCommerceCw_Util::includeTemplateFile('external_shipping_single', 
					array(
						'package' => $package,
			...
			foreach ($packages as $i => $package) {
				...
				EMSeCommerceCw_Util::includeTemplateFile('external_shipping_row', 
						array(
							'package' => $package,
							'availableMethods' => $package['rates'],
							'index' => $i,
		...
		$rows = ob_get_clean();
		ob_start();
		EMSeCommerceCw_Util::includeTemplateFile('external_shipping_table', array(
			'rows' => $rows,
	...
	public function getPossiblePaymentMethods(Customweb_Payment_ExternalCheckout_IContext $context){
		...
		foreach (EMSeCommerceCw_Util::getPaymentMethods(false) as $method) {
			$paymentMethods[] = EMSeCommerceCw_Util::getPaymentMehtodInstance($method);
		}
	...
	public function renderReviewPane(Customweb_Payment_ExternalCheckout_IContext $context, $renderConfirmationFormElements, $errorMessage){
		...
		$context->setVerificationHash(
				EMSeCommerceCw_Entity_ExternalCheckoutContext::computeVerificationHash($context->getInvoiceItems(), 
		...
		if ($context->getPaymentMethodMachineName() != null) {
			WC()->session->set('chosen_payment_method', 'EMSeCommerceCw_' . $context->getPaymentMethodMachineName());
		}
		...
		ob_start();
		EMSeCommerceCw_Util::includeTemplateFile('external_confirm_cart', $cartVariables);
		$variables['orderReview'] = ob_get_clean();
		
		$variables['showConfirm'] = $renderConfirmationFormElements;
		
		ob_start();
		EMSeCommerceCw_Util::includeTemplateFile('external_confirm', $variables);
		return ob_get_clean();
	}
	...
	public function validateReviewForm(Customweb_Payment_ExternalCheckout_IContext $context, Customweb_Core_Http_IRequest $request){
		...
		$context->updateFromCart(WC()->cart);
		$currentCartHash = EMSeCommerceCw_Entity_ExternalCheckoutContext::computeVerificationHash($context->getInvoiceItems(), 
				$context->getOrderAmountInDecimals(), $context->getCurrencyCode());
		if ($currentCartHash != $context->getVerificationHash()) {
			throw new Exception(__('Cart has been modified after displaying order review', 'woocommerce_emsecommercecw'));
		}
		$parameters = $request->getParameters();
	...
	public function renderAdditionalFormElements(Customweb_Payment_ExternalCheckout_IContext $context, $errorMessage){
		...
		ob_start();
		EMSeCommerceCw_Util::includeTemplateFile('external_additional', $variables);
		return ob_get_clean();
	}
	...
	protected function formatSubShippingTotal($cart){
		if ($cart->shipping_total > 0) {
			...
			return __('Free', 'woocommerce_emsecommercecw');
		}
	}
