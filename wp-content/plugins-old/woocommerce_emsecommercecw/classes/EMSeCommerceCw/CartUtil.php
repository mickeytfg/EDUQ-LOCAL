...
class EMSeCommerceCw_CartUtil {
	...
	public static function getInoviceItemsFromCart(WC_Cart $cart){
		...
		foreach ($wooCommerceItems as $wooItem) {
			...
			if (defined('WOOCOMMERCE_VERSION') && version_compare(WOOCOMMERCE_VERSION, '3.2') < 0) {
				$discountAmount = ($item->getAmountExcludingTax() / $quantity -
				 ...
				 if ($discountAmount > 0) {
				 	$discountItem = new Customweb_Payment_Authorization_DefaultInvoiceItem($sku . '-discount',
				 			__("Discount", "woocommerce_emsecommercecw") . ' ' . $name, $taxRate, $discountAmount * ($taxRate / 100 + 1), $quantity,
				...
				if ($discountAmount > 0) {
					$discountItem = new Customweb_Payment_Authorization_DefaultInvoiceItem($sku . '-discount',
							__("Discount", "woocommerce_emsecommercecw") . ' ' . $name, $taxRate, $discountAmount, $quantity,
							Customweb_Payment_Authorization_IInvoiceItem::TYPE_DISCOUNT);
					$items[] = $discountItem;
				}
			}
		...
		if ($cart->shipping_total > 0) {
			...
			$items[] = new Customweb_Payment_Authorization_DefaultInvoiceItem('shipping', __("Shipping", "woocommerce_emsecommercecw"),
