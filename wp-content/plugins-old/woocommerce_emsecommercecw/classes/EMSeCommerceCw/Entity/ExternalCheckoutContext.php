...
/**
 * @Entity(tableName = 'woocommerce_emsecommercecw_ecc')
 * @Filter(name = 'loadContextCookie', where = 'contextId = >contextId AND cookieKey = >cookieKey', orderBy = 'contextId')
 */
class EMSeCommerceCw_Entity_ExternalCheckoutContext extends Customweb_Payment_ExternalCheckout_AbstractContext {
	...
	protected function loadPaymentMethodByMachineName($machineName){
		$paymentMethods = EMSeCommerceCw_Util::getPaymentMethods(true);
		foreach($paymentMethods as $method) {
			$instance = EMSeCommerceCw_Util::getPaymentMehtodInstance($method);
	 ...
	 * @return EMSeCommerceCw_Entity_ExternalCheckoutContext
	 */
	public static function getContextById($id, $loadFromCache = true) {
		return EMSeCommerceCw_Util::getEntityManager()->fetch('EMSeCommerceCw_Entity_ExternalCheckoutContext', $id, $loadFromCache);
	}
	...
	public static function getReusableContextFromCookie($loadFromCache = true) {
		if (isset($_COOKIE['emsecommercecw-woocommerce-context-id']) && isset($_COOKIE['emsecommercecw-woocommerce-context-key'])) {
			try {
				$result = EMSeCommerceCw_Util::getEntityManager()->searchByFilterName('EMSeCommerceCw_Entity_ExternalCheckoutContext', 'loadContextCookie', array('>contextId' => $_COOKIE['emsecommercecw-woocommerce-context-id'], '>cookieKey' => $_COOKIE['emsecommercecw-woocommerce-context-key']), $loadFromCache);
				if (count($result) > 0) {
					$context = current($result);
					if ($context instanceof EMSeCommerceCw_Entity_ExternalCheckoutContext && $context->getState() == Customweb_Payment_ExternalCheckout_IContext::STATE_PENDING) {
						return $context;
					}
				}
			}
	...
	public function updateFromCart($cart) {
		...
		if (function_exists('get_woocommerce_currency')) {
			...
			$currencyCode = EMSeCommerceCw_Util::getShopOption('____module_name_____currency');
		}
		...
		if(EMSeCommerceCw_ConfigurationAdapter::getExternalCheckoutPlacement() == 'checkout') {
		...
		$this->setInvoiceItems(EMSeCommerceCw_CartUtil::getInoviceItemsFromCart($cart));
