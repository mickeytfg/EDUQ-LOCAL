...
class EMSeCommerceCw_RecurringOrderContextNew extends EMSeCommerceCw_AbstractRecurringOrderContext {
				...
				if (empty($initialOrderId)) {
					if (false !== $subscription->order) {
						if(method_exists($subscription->order, 'get_id')){
							$initialOrderId = $subscription->order->get_id();
						}
						else{
	...
	public function getInvoiceItems(){
		...
		if ($difference > 0) {
			...
			$items[] = new Customweb_Payment_Authorization_DefaultInvoiceItem('other-discount', 
					__('Other Discount', 'woocommerce_emsecommercecw'), $taxRate, abs($difference), 1, 
