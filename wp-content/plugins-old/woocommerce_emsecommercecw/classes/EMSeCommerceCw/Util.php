 ...
 *
 */
EMSeCommerceCw_Util::bootstrap();

require_once 'EMSeCommerceCw/Database/LinkAccessor.php';
require_once 'EMSeCommerceCw/EntityManager.php';
...
require_once 'EMSeCommerceCw/ContextCleanUpBean.php';
...
require_once 'EMSeCommerceCw/TransactionCleanUpBean.php';
require_once 'EMSeCommerceCw/Util.php';
require_once 'Customweb/Core/Url.php';
require_once 'EMSeCommerceCw/Entity/PaymentCustomerContext.php';
require_once 'EMSeCommerceCw/LayoutRenderer.php';
...
class EMSeCommerceCw_Util {
	 ...
	 * @return EMSeCommerceCw_PaymentMethod
	 */
	...
	public static function getPluginUrl($controller, array $params = array(), $action = null){
		...
		$urlString = get_permalink(get_option('woocommerce_emsecommercecw_page'));
		global $sitepress;
		if($sitepress != null && is_callable(array($sitepress, "convert_url"))){
			$urlString = $sitepress->convert_url($urlString, $params['wpml-lang']);
		}
		...
		return apply_filters('woocommerce_emsecommercecw_plugin_url', $complete, $url->getBaseUrl() . $url->getPath(), 
				$url->getQueryAsArray());
	}
	...
	private static function checkRequirements() {
		...
		$errors = array();
			...
			$errors[] = sprintf(__("Woocommerce has to be active.", "woocommerce_emsecommercecw"));
		}
		
		if(!empty($errors)){
			$title = __('Could not activate plugin WooCommerce EMSeCommerceCw', 'woocommerce_emsecommercecw');
			$message = '<h1><strong>'.$title.'</strong></h1><br/>'.
					'<h3>'.__('Please check the following requirements before activating:', 'woocommerce_emsecommercecw').'</h3>'.
					'<ul><li>'.
	...
	public static function installPlugin(){
		...
		$manager = new Customweb_Database_Migration_Manager(self::getDriver(), dirname(__FILE__) . '/Migration/', 
				$wpdb->prefix . 'woocommerce_emsecommercecw_schema_version');
		$manager->migrate();
		
		//Create Page
		$optionValue = get_option('woocommerce_emsecommercecw_page');
		$pageContent = '[woocommerce_emsecommercecw]';
		$pageSlug = 'woo_emsecommercecw';
		$pageTitle = 'EMS eCommerce Gateway Checkout';
		...
		update_option('woocommerce_emsecommercecw_page', $pageId);
		
		// Append order status for pending payments
		if (!term_exists('emsecommercecw-pending', 'shop_order_status')) {
			wp_insert_term('emsecommercecw-pending', 'shop_order_status',
					array(
						'description' => 'Orders with that order status are currently in the checkout of EMS eCommerce Gateway.',
						'slug' => 'emsecommercecw-pending'
					));
		}
	}
	...
	public static function includeTemplateFile($templateName, $variables = array()){
		...
		$templateName = 'emsecommercecw_' . $templateName;
	...
	public static function createContainer(){
		if (self::$container === null) {
			...
			// @formatter:off
			$storage = new Customweb_Storage_Backend_Database(self::getEntityManager(), self::getDriver(), 'EMSeCommerceCw_Entity_Storage');
			$provider->addObject(EMSeCommerceCw_ContextRequest::getInstance())
				->addObject(self::getEntityManager())
				->addObject(self::getDriver())
				->addObject(new EMSeCommerceCw_LayoutRenderer())
				->addObject(new Customweb_Cache_Backend_Memory())
				->add('databaseTransactionClassName', 'EMSeCommerceCw_Entity_Transaction')
				->addObject(self::getAssetResolver())
				->addObject($storage)
				->addObject(new EMSeCommerceCw_ContextCleanUpBean(self::getEntityManager()))
				->addObject(new EMSeCommerceCw_TransactionCleanUpBean(self::getEntityManager()));
			// @formatter:om
	...
	public static function getEntityManager(){
		if (self::$entityManager === null) {
			...
			self::$entityManager = new EMSeCommerceCw_EntityManager(self::getDriver(), $cache);
		}
	...
	public static function getAssetResolver(){
		$simple = array();
		$simple[] = new Customweb_Asset_Resolver_Simple(get_template_directory(). '/woocommerce_emsecommercecw/', null,
				array(
					'application/x-phtml'
				));
		...
		$simple[] = new Customweb_Asset_Resolver_Simple(get_template_directory(). '/woocommerce_emsecommercecw/', get_stylesheet_directory_uri() . '/woocommerce_emsecommercecw/');
	...
	public static function getDriver(){
		if (self::$driver === null) {
			global $wpdb;
			$link = EMSeCommerceCw_Database_LinkAccessor::getEMSeCommerceCwLink($wpdb);
	...
	public static function getPaymentCustomerContext($customerId){
		...
		if (!isset(self::$paymentCustomerContexts[$customerId])) {
			$entities = self::getEntityManager()->searchByFilterName('EMSeCommerceCw_Entity_PaymentCustomerContext', 'loadByCustomerId', 
					array(
			...
			if (count($entities) > 0) {
				...
				$context = new EMSeCommerceCw_Entity_PaymentCustomerContext();
	...
	public static function persistPaymentCustomerContext(Customweb_Payment_Authorization_IPaymentCustomerContext $context){
		if ($context instanceof EMSeCommerceCw_Entity_PaymentCustomerContext) {
	...
	public static function thankYouPageHtml($orderId){
		...
		if (!empty($paymentInformation)) {
			echo '<div class="woocommerce_emsecommercecw-payment-information" id="woocommerce_emsecommercecw-payment-information">';
			echo "<h2>" . __('Payment Information', 'woocommerce_emsecommercecw') . "</h2>";
	...
	public static function orderEmailHtml($order, $sent_to_admin, $plain_text = false){
		...
		$transactions = self::getTransactionsByPostId($order_id);
		if(empty($transactions)){
			$transactions = self::getTransactionsByOrderId($order_id);
		}
		foreach ($transactions as $transaction) {
			if ($transaction->getTransactionObject() !== null && $transaction->getTransactionObject()->isAuthorized()) {
				$transactionObject = $transaction->getTransactionObject();
		...
		if(!empty($paymentInformation)) {
			echo '<div class="woocommerce_emsecommercecw-email-payment-information" id="woocommerce_emsecommercecw-email-payment-information">';
			echo "<h2>" . __('Payment Information', 'woocommerce_emsecommercecw') . "</h2>";
			echo $paymentInformation;
			echo '</div>';
		}
	 ...
	 * @return EMSeCommerceCw_Entity_Transaction The matching transactions for the given transaction id
	 */
	public static function getTransactionById($id, $cache = true){
		return self::getEntityManager()->fetch('EMSeCommerceCw_Entity_Transaction', $id, $cache);
	}
	 ...
	 * @param boolean $cache load from cache
	 * @return EMSeCommerceCw_Entity_Transaction The matching transactions for the given transactionNumber
	 */
	public static function getTransactionByTransactionNumber($number, $cache = true){
		$transactions = self::getEntityManager()->searchByFilterName('EMSeCommerceCw_Entity_Transaction', 'loadByExternalId', 
				array(
	 ...
	 * @param boolean $cache load from cache
	 * @return EMSeCommerceCw_Entity_Transaction[] The matching transactions for the given order id
	 */
	public static function getTransactionsByOrderId($orderId, $cache = true){
		class_exists('WC_Order');
		self::getPaymentMethods(true);
		return self::getEntityManager()->searchByFilterName('EMSeCommerceCw_Entity_Transaction', 'loadByOrderId', 
				array(
					'>orderId' => $orderId 
				), $cache);
	}
	...
	public static function getTransactionsByPostId($postId, $cache = true){
		class_exists('WC_Order');
		self::getPaymentMethods(true);
		return self::getEntityManager()->searchByFilterName('EMSeCommerceCw_Entity_Transaction', 'loadByPostId',
				array(
					'>postId' => $postId
				), $cache);
	}
	...
	public static function getAliasTransactions($userId, $paymentMethod){
		...
		$aliases = array();
		$entities = self::getEntityManager()->search('EMSeCommerceCw_Entity_Transaction', 
...
;
	...
	public static function computeTransactionValidateHash(EMSeCommerceCw_Entity_Transaction $transaction) {
	...
	public static function checkToken(EMSeCommerceCw_Entity_ExternalCheckoutContext $context, $parameters =array()) {
		if (!empty($parameters['emsecommercecw-context-id'])) {
	...
	public static function createUser($email, $password, $firstname = null, $lastname = null) {
		...
		if(is_wp_error($userId)){
			throw new Exception(__('Could not create user.', 'woocommerce_emsecommercecw').' '.$userId->get_error_message());
		}
		return $userId;	
	}
...
if(function_exists('wc_get_page_id')) {
	...
	$id = EMSeCommerceCw_Util::getShopOption('woocommerce_checkout_page_id');
