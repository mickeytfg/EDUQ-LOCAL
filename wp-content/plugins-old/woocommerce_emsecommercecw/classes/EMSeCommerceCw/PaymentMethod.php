...
require_once 'EMSeCommerceCw/AbstractPaymentMethod.php';
require_once 'EMSeCommerceCw/RecurringOrderContextNew.php';
require_once 'Customweb/Payment/Authorization/Recurring/IAdapter.php';
require_once 'EMSeCommerceCw/PaymentMethodWrapper.php';
require_once 'EMSeCommerceCw/RecurringTransactionContext.php';
require_once 'EMSeCommerceCw/RecurringOrderContext.php';
 ...
 */
class EMSeCommerceCw_PaymentMethod extends EMSeCommerceCw_AbstractPaymentMethod {
	...
	public function __construct(){
		...
		// Workaround: When some setting is stored all EMSeCommerceCw methods are
	...
	public function admin_options(){
		$output = '<h3>' . __($this->admin_title, 'woocommerce_emsecommercecw') . '</h3>';
	...
	public function scheduledSubscriptionPayment($amountToCharge, $order, $productId){
		...
		$emsecommercecw_recurring_process_failure = NULL;
		try {
			$adapter = EMSeCommerceCw_Util::getAuthorizationAdapter(
					Customweb_Payment_Authorization_Recurring_IAdapter::AUTHORIZATION_METHOD_NAME);
			
			$orderContext = new EMSeCommerceCw_RecurringOrderContext($order, new EMSeCommerceCw_PaymentMethodWrapper($this), $amountToCharge, 
					$productId);
			$dbTransaction = $this->newDatabaseTransaction($orderContext);
			$transactionContext = new EMSeCommerceCw_RecurringTransactionContext($dbTransaction, $orderContext);
			...
			$errorMessage = __('Subscription Payment Failed with error:', 'woocommerce_emsecommercecw') . $e->getMessage();
			$emsecommercecw_recurring_process_failure = $errorMessage;
			WC_Subscriptions_Manager::process_subscription_payment_failure_on_order($order, $product_id);
			return;
		}
		try {
			$adapter->process($transaction);
			EMSeCommerceCw_Util::getTransactionHandler()->persistTransactionObject($transaction);
	...
	public function scheduledSubscriptionPaymentNew($amountToCharge, $order){
		...
		if(EMSeCommerceCw_Util::getAuthorizedTransactionByPostId($orderId) != null){
			return;	
		}
		try {			
			$adapter = EMSeCommerceCw_Util::getAuthorizationAdapter(
			...
			$orderContext = new EMSeCommerceCw_RecurringOrderContextNew($order, new EMSeCommerceCw_PaymentMethodWrapper($this), 
					$amountToCharge);
			update_post_meta($orderId, '_emsecommercecw_pending_state', 'yes' );
	...
	public function process_admin_options(){
		global $woocommerce_emsecommercecw_isProcesssing;
		if ($woocommerce_emsecommercecw_isProcesssing) {
			return true;
		}
		$woocommerce_emsecommercecw_isProcesssing = true;
	...
	public function validate_file_field($key){
		...
		if (!file_exists($upload_dir['basedir'] . '/woocommerce_emsecommercecw')) {
			$oldmask = umask(0);
			mkdir($upload_dir['basedir'] . '/woocommerce_emsecommercecw', 0777, true);
			umask($oldmask);
		}
		...
		$targetPath = $upload_dir['basedir'] . '/woocommerce_emsecommercecw/' . $filename . '.' . $fileExtension;
		...
		if ($rs) {
			...
			woocommerce_emsecommercecw_admin_show_message('Unable to upload file for setting "' . $setting['title'] . '".', 'error');
			return $value;
		}
	}
...
<tr valign="top">
	...
	<td class="forminp">
		...
		echo __('Current File: ', 'woocommerce_emsecommercecw') . esc_attr($filename);
		?><br />
		<fieldset>
		...
		name="<?php echo esc_attr( $field.'_reset' ); ?>" value="reset" /><?php echo __('Reset', 'woocommerce_emsecommercecw'); ?><br />
	</td>
</tr>
<?php
		return ob_get_clean();
	...
	protected function getOrderStatusOptions($statuses = array()){
		if (defined('WOOCOMMERCE_VERSION') && version_compare(WOOCOMMERCE_VERSION, '2.2.0') >= 0) {
			...
			foreach ($statuses as $k => $value) {
				$orderStatuses[$k] = __($value, 'woocommerce_emsecommercecw');
			}
	...
	protected function getCompatibilityFormFields(){
		...
		if (is_plugin_active('woocommerce-german-market/WooCommerce-German-Market.php')) {
			$extra .= '<div class="emsecommercecw-requires-second-run"></div>';
		}
		return $extra;
	}
	...
	public function process_payment($order_id){
		...
		$order = EMSeCommerceCw_Util::loadOrderObjectById($order_id);
		...
		$order->add_order_note(
				__('The customer is now in the payment process of EMS eCommerce Gateway.', 'woocommerce_emsecommercecw'));
		update_post_meta($order_id, '_emsecommercecw_pending_state', 'yes' );
		...
		if (is_ajax()) {
				...
				if (is_array($result)) {
					...
					wp_send_json(
							array(
								'result' => 'success',
								'data' => $result .
										 "<script type=\"text/javascript\"> var backToCheckoutCw = jQuery('#emsecommercecw-back-to-checkout'); jQuery('form.checkout').replaceWith(jQuery('#emsecommercecw-payment-container')); jQuery('#emsecommercecw-payment-container').after(backToCheckoutCw); jQuery('.woocommerce-info').remove(); jQuery('.cw-external-checkouts').remove(); jQuery('html, body').animate({ scrollTop: (jQuery('#emsecommercecw-payment-container').offset().top-150) }, '1000');</script>" 
							));
					die();
				}
			}
			...
			wp_redirect(
					EMSeCommerceCw_Util::getPluginUrl("payment", 
							array(
								'cwoid' => $order_id,
								'cwot' => EMSeCommerceCw_Util::computeOrderValidationHash($order_id),
	...
	protected function destroyCheckoutId(){
		...
		if($sessionHandler != null){
			if(method_exists($sessionHandler, 'set')){
				$sessionHandler->set('EMSeCommerceCwCheckoutId', null);
			}
			else{
				$sessionHandler->EMSeCommerceCwCheckoutId = null;
			}
		}
	}
