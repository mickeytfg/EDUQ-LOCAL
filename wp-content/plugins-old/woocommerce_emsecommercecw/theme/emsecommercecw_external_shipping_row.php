...
<tr class="emsecommercecw-external-checkout-shipping-row">
<th><?php
	printf( __( 'Shipping #%d', 'woocommerce_emsecommercecw' ), $index + 1 );
?>
</th>
	<td>
		<?php if ( count( $availableMethods ) > 0) : ?>
			...
			<?php foreach ( $availableMethods as $method ) : ?>
				<li>
					...
					<label for="shipping_method_<?php echo $index; ?>_<?php echo sanitize_title( $method->id ); ?>"><?php echo $method->label; ?>: <?php echo woocommerce_emsecommercecw_format_shipping_amount_like_shop($method);?></label>
				</li>
			<?php endforeach; ?>
			</ul>
		<?php else : ?>
		...
		<?php
			...
			echo '<p class="emsecommercecw-external-checkout-contents"><small>' . __( 'Shipping', 'woocommerce_emsecommercecw' ) . ': ' . implode( ', ', $product_names ) . '</small></p>';
		?>
