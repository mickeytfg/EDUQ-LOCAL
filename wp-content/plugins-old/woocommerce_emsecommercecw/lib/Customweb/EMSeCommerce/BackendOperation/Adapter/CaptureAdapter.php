 ...
 * @Bean
 */
class Customweb_EMSeCommerce_BackendOperation_Adapter_CaptureAdapter extends Customweb_EMSeCommerce_AbstractAdapter implements 
	...
	public function partialCapture(Customweb_Payment_Authorization_ITransaction $transaction, $items, $close){
		...
		Customweb_EMSeCommerce_Util::setTxTypeForPaymentMethod($paymentMethod, Customweb_EMSeCommerce_IConstants::OPERATION_CAPTURE, 
				$ipgTransaction);
		$ipgTransaction->setTransactionDetails($ipgTransactionDetails)->setPayment($ipgPayment);
		...
		$service->setClientCertificate(Customweb_EMSeCommerce_Util::getCertificate($this->getConfiguration(), $transaction->isMoto()))->setClientCertificatePassphrase(
				$this->getCertificatePassphrase($isMoto));
		...
		if (empty($errorMessage)) {
			...
			$transaction->setCaptureTDate($response->getTDate()->get());
		}
		else {
