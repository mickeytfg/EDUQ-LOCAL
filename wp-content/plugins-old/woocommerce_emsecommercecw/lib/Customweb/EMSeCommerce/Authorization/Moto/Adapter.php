...
require_once 'Customweb/EMSeCommerce/Authorization/PaymentPage/Adapter.php';
...
require_once 'Customweb/EMSeCommerce/Authorization/Hidden/Adapter.php';
...
class Customweb_EMSeCommerce_Authorization_Moto_Adapter extends Customweb_EMSeCommerce_Authorization_AbstractAdapter
implements Customweb_Payment_Authorization_Moto_IAdapter {
	...
	public function getParameters(Customweb_Payment_Authorization_ITransaction $transaction){
		...
		$parameters = array();
		if($adapter instanceof Customweb_EMSeCommerce_Authorization_Hidden_Adapter)
		{
		...
		$parameters['hash'] = Customweb_EMSeCommerce_Util::calculateHash($parameters, $this->getConfiguration()->getMotoConnectSharedSecret());
	...
	protected function getAdapterInstanceByPaymentMethod(Customweb_Payment_Authorization_IPaymentMethod $paymentMethod) {
		...
		switch (strtolower($configuredAuthorizationMethod)) {
			...
			case strtolower(Customweb_Payment_Authorization_Hidden_IAdapter::AUTHORIZATION_METHOD_NAME):
				$adapter = new Customweb_EMSeCommerce_Authorization_Hidden_Adapter($this->getContainer());
				break;
	
			case strtolower(Customweb_Payment_Authorization_PaymentPage_IAdapter::AUTHORIZATION_METHOD_NAME):
				$adapter = new Customweb_EMSeCommerce_Authorization_PaymentPage_Adapter($this->getContainer());
				break;
			default:
