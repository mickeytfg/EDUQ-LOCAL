 ...
 * @Bean
 */
class Customweb_EMSeCommerce_Authorization_AuthorizationHandler {
	private $container;

	public function __construct(Customweb_DependencyInjection_IContainer $container){
		$this->container = new Customweb_EMSeCommerce_Container($container);
	}
	...
	public function handleAuthorizationWithHashCheck(Customweb_EMSeCommerce_Authorization_Transaction $transaction, array $parameters){
		/*
		 * @var Customweb_EMSeCommerce_Authorization_Transaction $transaction
		 */
		...
		if (isset($parameters['notification_hash'])) {
			...
			if ($transaction->isMoto()) {
				$computedHash = Customweb_EMSeCommerce_Util::calculateNotificationHash($parameters, 
			...
			if(!isset($parameters['approval_code'])){
				$backendMessage = Customweb_I18n_Translation::__('The return parameters from EMS eCommerce Gateway are missing. Please check the Force Non SSL Notification Setting and your rewrite rules.');
			}
	...
	public function handleAuthorizationNoHashCheck(Customweb_EMSeCommerce_Authorization_Transaction $transaction, array $parameters){
		$transaction->setAuthorizationParameters($parameters);
		if (isset($parameters['oid'])) {
			$transaction->setPaymentId($parameters['oid']);
		}
		...
		switch (substr($approvalCode, 0, 1)) {
			case 'Y':
				...
				if (isset($parameters['response_code_3dsecure'])) {
					...
					switch ($parameters['response_code_3dsecure']) {
						...
						case 2: //successful without AVV
							...
							$transaction->setThreeDSecureState(Customweb_EMSeCommerce_IConstants::THREE_D_SUCCESS);
							break;
						
						case 4: //Authorization Attempted (card may be not enrolled)
							$transaction->authorize();
							if (in_array(Customweb_EMSeCommerce_IConstants::THREE_D_AUTH_ATTEMPTED, 
									$this->getConfiguration()->getUncertain3dstates())) {
								$transaction->setAuthorizationUncertain();
							}
							$transaction->setThreeDSecureState(Customweb_EMSeCommerce_IConstants::THREE_D_AUTH_ATTEMPTED);
							break;
						
						case 5: //Directory Server not responding
							$transaction->authorize();
							if (in_array(Customweb_EMSeCommerce_IConstants::THREE_D_DIR_SERVER, 
									$this->getConfiguration()->getUncertain3dstates())) {
								$transaction->setAuthorizationUncertain();
							}
							$transaction->setThreeDSecureState(Customweb_EMSeCommerce_IConstants::THREE_D_DIR_SERVER);
							break;
						
						case 6: //Authorization Control Server not responding
							$transaction->authorize();
							if (in_array(Customweb_EMSeCommerce_IConstants::THREE_D_AUTH_SERVER, 
									$this->getConfiguration()->getUncertain3dstates())) {
								$transaction->setAuthorizationUncertain();
							}
							$transaction->setThreeDSecureState(Customweb_EMSeCommerce_IConstants::THREE_D_AUTH_SERVER);
							break;
						
						case 7: //Not enrolled
							$transaction->authorize();
							if (in_array(Customweb_EMSeCommerce_IConstants::THREE_D_NOT_ENROLLED, 
									$this->getConfiguration()->getUncertain3dstates())) {
								$transaction->setAuthorizationUncertain();
							}
							$transaction->setThreeDSecureState(Customweb_EMSeCommerce_IConstants::THREE_D_NOT_ENROLLED);
							break;
						...
						default:
							...
							$transaction->setThreeDSecureState(Customweb_EMSeCommerce_IConstants::THREE_D_FAILED);
							break;
					}
				}
				else {
				...
				if (!$supportDeferred ||
						 (isset($parameters['txntype']) && $parameters['txntype'] == Customweb_EMSeCommerce_IConstants::OPERATION_SALE)) {
					$transaction->capture();
				}
				break;
			case '?':
				...
				if (isset($parameters['response_code_3dsecure'])) {
					...
					switch ($parameters['response_code_3dsecure']) {
						case 1: //successful
						case 2: //successful without AVV
							$transaction->setThreeDSecureState(Customweb_EMSeCommerce_IConstants::THREE_D_STATUS_SUCCESS);
							break;
						...
						case 5: //Directory Server not responding
							$transaction->setThreeDSecureState(Customweb_EMSeCommerce_IConstants::THREE_D_STATUS_DIR_ERROR);
							break;
						
						case 6: //Authorization Control Server not responding
							$transaction->setThreeDSecureState(Customweb_EMSeCommerce_IConstants::THREE_D_STATUS_AUTH_ERROR);
							break;
						
						case 7: //Not enrolled
							$transaction->setThreeDSecureState(Customweb_EMSeCommerce_IConstants::THREE_D_STATUS_NOT_ENROLLED);
							break;
						...
						default:
							$transaction->setAuthorizationFailed(Customweb_I18n_Translation::__('3d Secure Verification failed'));
							$transaction->setThreeDSecureState(Customweb_EMSeCommerce_IConstants::THREE_D_STATUS_FAILED);
							return $this->finalizeAuthorizationRequest($transaction);
					}
				}
	...
	public function handleFailedTransaction(Customweb_EMSeCommerce_Authorization_Transaction $transaction, array $parameters){
		$paymentMethod = $this->getContainer()->getPaymentMethodByTransaction($transaction);
		//failed
		$reason = '';
		if (isset($parameters['fail_reason'])) {
	...
	public function isDeferredCapturingSupported(Customweb_EMSeCommerce_Authorization_Transaction $transaction){
		$paymentMethod = $this->getContainer()->getPaymentMethodByTransaction($transaction);
