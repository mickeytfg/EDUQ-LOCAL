...
require_once 'Customweb/EMSeCommerce/AbstractParameterBuilder.php';
require_once 'Customweb/EMSeCommerce/Util.php';


abstract class Customweb_EMSeCommerce_Authorization_AbstractParameterBuilder extends Customweb_EMSeCommerce_AbstractParameterBuilder {
	
	private $formData = array();
	
	/**
	 *
	 * @var Customweb_EMSeCommerce_Authorization_Transaction
	 */
	...
	/**
	 *
	 * @return Customweb_EMSeCommerce_Authorization_Transaction
	 */
	...
	protected function getPaymentParameters(){
		...
		$parameters['currency'] = Customweb_EMSeCommerce_Util::getNumericCurrencyCode($this->getOrderContext()->getCurrencyCode());
		return $parameters;
	}
	...
	protected function getLanguageParameter(){
		...
		$language = Customweb_Core_Util_Language::getCleanLanguageCode($this->getOrderContext()->getLanguage(),
				Customweb_EMSeCommerce_Util::$supportedLanguages);
	...
	protected function getProcessAuthorizationUrl(){
		...
		// EMSeCommerce is encoding the string on there side again. If we pass it in proper
...
// 			$params['recurringInstallmentFrequency'] = '1';
	...
	protected function getSolvencyVerficiationParameters() {
		...
		if($paymentMethod->isSolvencyCheckActive()) {
					$parameters = array();
			$company = $this->getOrderContext()->getBillingCompanyName();
			if(!empty($company)) {
				$parameters['bcompany'] = $company;
			}
			$parameters['bname'] =$this->getOrderContext()->getBillingFirstName() . ' ' . $this->getOrderContext()->getBillingLastName();
			$parameters['baddr1'] = Customweb_Core_String::_($this->getOrderContext()->getBillingStreet())->substring(0, 30)->toString();
			$parameters['bcity'] = Customweb_Core_String::_($this->getOrderContext()->getBillingCity())->substring(0, 30)->toString();
			$parameters['bstate'] = Customweb_Core_String::_($this->getOrderContext()->getBillingState())->substring(0, 30)->toString();
			$parameters['bcountry'] = $this->getOrderContext()->getBillingCountryIsoCode();
			$parameters['bzip'] = $this->getOrderContext()->getBillingPostCode();
			$parameters['phone'] = Customweb_Core_String::_($this->getOrderContext()->getBillingPhoneNumber())->substring(0, 20)->toString();
			$parameters['email'] = Customweb_Core_String::_($this->getOrderContext()->getBillingEMailAddress())->substring(0, 45)->toString();
	...
	protected function getPaymentMethod() {
		$factory = $this->getContainer()->getBean('Customweb_EMSeCommerce_Method_Factory');
