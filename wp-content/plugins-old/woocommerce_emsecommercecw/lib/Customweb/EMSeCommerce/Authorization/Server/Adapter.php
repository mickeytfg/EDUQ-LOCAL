 ...
 * @Bean
 */
class Customweb_EMSeCommerce_Authorization_Server_Adapter extends Customweb_EMSeCommerce_Authorization_AbstractAdapter implements 
	...
	public function processAuthorization(Customweb_Payment_Authorization_ITransaction $transaction, array $parameters){
		if (!($transaction instanceof Customweb_EMSeCommerce_Authorization_Transaction)) {
		}
		try {
			...
			$service = new Customweb_EMSeCommerce_SoapService($this->getAPIUser(false), $this->getAPIPassword(false), $this->getAPIUrl());
			$service->setClientCertificate(Customweb_EMSeCommerce_Util::getCertificate($this->getConfiguration(), false));
			...
			$authorizationHandler = $this->getContainer()->getBean('Customweb_EMSeCommerce_Authorization_AuthorizationHandler');
