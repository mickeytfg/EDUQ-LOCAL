...
require_once 'Customweb/EMSeCommerce/Authorization/PaymentPage/ParameterBuilder.php';
 ...
 * @Bean
 */
class Customweb_EMSeCommerce_Authorization_PaymentPage_Adapter extends Customweb_EMSeCommerce_Authorization_AbstractAdapter
implements Customweb_Payment_Authorization_PaymentPage_IAdapter
{
	...
	private function generateUrl(Customweb_Payment_Authorization_ITransaction $transaction, array $formData) {
		...
		try {
			$url = new Customweb_Core_Url($this->getConfiguration()->getBaseUrl().Customweb_EMSeCommerce_IConstants::URL_CONNECT);
			$builder = new Customweb_EMSeCommerce_Authorization_PaymentPage_ParameterBuilder($transaction, $this->getContainer(), $formData);
