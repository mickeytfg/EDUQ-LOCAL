...
class Customweb_EMSeCommerce_ExternalCheckout_MasterPass_RedirectionParameterBuilder extends Customweb_EMSeCommerce_AbstractParameterBuilder {
	...
	public function build(){
		...
		$parameters['hashExtended'] = Customweb_EMSeCommerce_Util::calculateExtendedHash($parameters,$this->getConfiguration()->getConnectSharedSecret());
		return $parameters;
	}
	...
	protected function getPaymentParameters(){
		...
		$parameters['currency'] = Customweb_EMSeCommerce_Util::getNumericCurrencyCode($this->getCheckoutContext()->getCurrencyCode());
		return $parameters;
	}
	...
	protected function getOperationModeParameter(){
		...
		$captureType = Customweb_EMSeCommerce_IConstants::OPERATION_SALE;
		if (strtolower($capturingMode) == 'deferred') {
			$captureType = Customweb_EMSeCommerce_IConstants::OPERATION_AUTHORISATION;
		}
		return array(
			'txntype' => $captureType,
			'mode' => Customweb_EMSeCommerce_IConstants::DATA_ALL,
			'reviewOrder' => 'true',
