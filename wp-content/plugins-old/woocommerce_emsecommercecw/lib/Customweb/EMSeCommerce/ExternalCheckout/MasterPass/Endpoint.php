...
require_once 'Customweb/EMSeCommerce/ExternalCheckout/MasterPass/RedirectionParameterBuilder.php';
...
require_once 'Customweb/EMSeCommerce/ExternalCheckout/MasterPass/ConfirmationParameterBuilder.php';
 ...
 */
class Customweb_EMSeCommerce_ExternalCheckout_MasterPass_Endpoint extends Customweb_Payment_ExternalCheckout_AbstractCheckoutEndpoint {
	...
	public function redirectAction(Customweb_Core_Http_IRequest $request){
		...
		try {
			...
			$templateContext->addVariable('url', 
					Customweb_Core_Url::_($this->container->getConfiguration()->getBaseUrl() . Customweb_EMSeCommerce_IConstants::URL_CONNECT)->toString());
	...
	public function confirmationAction(Customweb_Core_Http_IRequest $request){
		...
		try {
				...
				if ($additionalFormErrorMessage === null) {
					try {
						...
						if (!$transaction->isAuthorized() && !$transaction->isAuthorizationFailed()) {
							...
							$parameterBuilder = new Customweb_EMSeCommerce_ExternalCheckout_MasterPass_ConfirmationParameterBuilder($this->getContainer(),
			...
			$templateContext->addVariable('javascript', 
					$this->getAjaxJavascript('.emsecommerce-masterpass-shipping-pane', '.emsecommerce-masterpass-confirmation-pane'));
		...
		if ($context->getState() == Customweb_Payment_ExternalCheckout_IContext::STATE_COMPLETED) {
			...
			for ($i = 0; $i < 5; $i++) {
				try {
					...
					$authorizationHandler = $this->container->getBean('Customweb_EMSeCommerce_Authorization_AuthorizationHandler');
	...
	private function getRedirectionParameters(Customweb_Payment_ExternalCheckout_IContext $context, $token){
		$builder = new Customweb_EMSeCommerce_ExternalCheckout_MasterPass_RedirectionParameterBuilder($context, $this->container, $token);
	...
	/**
	 *
	 * @return Customweb_EMSeCommerce_Debitor_Manager
	 */
	protected function getDebitorManager(){
		return $this->getContainer()->getBean('Customweb_EMSeCommerce_Debitor_Manager');
	}

	/**
	 *
	 * @return Customweb_EMSeCommerce_Method_Factory
	 */
	protected function getMethodFactory(){
		return $this->getContainer()->getBean('Customweb_EMSeCommerce_Method_Factory');
	}

	protected function getPaymentMethodByTransaction(Customweb_EMSeCommerce_Authorization_Transaction $transaction){
